...
template<class T, class U> class DDlist;
template<class T, class U> class DDlink;
template<class T, class U> class DDlist_iterator;
...
public:
    ...
    DDlist();	//constructor @
    DDlist(const DDlist<T,U> & l);	//copy constructor @
    ~DDlist();	//deconstructor @

    bool empty() const; 	//@
    unsigned int size() const;	//@
    dditerator begin() const;	//return iterator at fist link  @
    dditerator end() const;	//return iterator at last link @
//    iterator end() const;	//return iterator at last link
//    virtual void insert(const T & x); //insert given T value
...
//    iterator search(const T & x); //search for value and returns an iterator position
    ...
    void push_back(const T & x, const U & y);	//@

private:
    DDlink<T,U> * first_link; //link at beginning of list
    DDlink<T,U> * last_link;	// link at end of list
    unsigned int my_size;
};
...
template<class T, class U>
DDlist<T,U>::DDlist(const DDlist & l)
{
    first_link = 0;
    last_link = 0;
//uses push_back to insert incoming link of list to the end of target list
    for(DDlink<T,U> * current = l.first_link; current != l.last_link; current = current->next_link)
        push_back(current->value, current->uvalue);
    push_back(l.last_link->value, l.last_link->uvalue);
}

template<class T, class U>
DDlist<T,U>::~DDlist()
{
    DDlink<T,U> * first = first_link;
    while(first != 0){
	    DDlink<T,U> * next = first->next_link;
	delete first;
	first = next;
    }
}
...
template<class T, class U>
unsigned int DDlist<T,U>::size() const
{
    return my_size;
}

template<class T, class U>
typename DDlist<T,U>::dditerator DDlist<T,U>::begin() const
{
    return dditerator(first_link);
}

template<class T, class U>
typename DDlist<T,U>::dditerator DDlist<T,U>::end() const
{
    return dditerator(last_link);
}

template<class T, class U>
void DDlist<T,U>::push_back(const T & x, const U & y)
{
    DDlink<T,U> * new_link  = new DDlink<T,U>(x,y);
//if empty, first link and last link equal new link @param T value
    if (empty()){
	first_link = last_link = new_link;
	my_size++;
...
template<class T, class U>
void DDlist<T,U>::read()
{
    if (empty())
        cout << "list is empty" << endl;
    else {
        DDlink<T,U> * rlink = first_link;
	while (rlink != last_link){
	    cout << "(" << rlink->value << ", " << rlink->uvalue << ") " << endl;
	    rlink = rlink->next_link;
	}
	cout << "(" << rlink->value << ", " << rlink->uvalue << ") " << endl;
    }
...
{
private:
    DDlink(const T & x, const U & y): value(x), uvalue(y), next_link(0){}	//@

    T value;
    U uvalue;

    DDlink<T,U> * next_link;

    friend class DDlist<T,U>;	//@
    friend class DDlist_iterator<T,U>;	//@
};

//___________________________________________________________________

template<class T, class U>
class DDlist_iterator
{
public:
    typedef DDlist_iterator<T,U> dditerator;

    DDlist_iterator(DDlink<T,U> * source_link): current_link(source_link){}	//@
    DDlist_iterator(): current_link(0){}	//@
    DDlist_iterator(DDlist_iterator<T,U> * source_iterator): current_link(source_iterator.current_link){}	//@

    T & operator*();	//return value at link	@
    U & operator~();    //return uvalue at link @
    dditerator & operator++(); // pre-increment	@
    bool operator!=(const dditerator & rhs) const; // @

protected:
    DDlink<T,U> * current_link;

    friend class DDlist<T,U>;	//@
};

template<class T, class U>
T & DDlist_iterator<T,U>::operator*()
{
    return current_link->value;
}

template<class T, class U>
U & DDlist_iterator<T,U>::operator~()
{
    return current_link->uvalue;
}


template<class T, class U>
DDlist_iterator<T,U> & DDlist_iterator<T,U>::operator++()
{
    current_link = current_link->next_link;
    return *this;
}
...
template<class T, class U>
bool DDlist_iterator<T,U>::operator!=(const dditerator & rhs) const
{
